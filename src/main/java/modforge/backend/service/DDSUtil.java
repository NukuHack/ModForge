...
public class DDSUtil {
	...
	private DDSUtil() {} // Prevent instantiation
	...
	private static ImgData readHeader(DataInputStream dis, boolean isLittleEdian) throws IOException {
		...
		if (read < 128) throw new IOException("DDS header too short");
		...
		if (size != 124) throw new IOException("DDS header size invalid");
		...
		if (pfSize != 32) throw new IOException("Pixel format size invalid");
	...
	private static void decompressDXT1(ImgData data, byte[] out) {
		...
		for (int y = 0; y < height; y += 4) {
			for (int x = 0; x < width; x += 4) {
				...
				long bits = ((long)(blockData[blockIndex + 7] & 0xFF) << 24) |
						((blockData[blockIndex + 6] & 0xFF) << 16) |
						((blockData[blockIndex + 5] & 0xFF) << 8) |
						(blockData[blockIndex + 4] & 0xFF);
	...
	private static void decompressDXT3(ImgData data, byte[] out) {
		...
		for (int y = 0; y < height; y += 4) {
			for (int x = 0; x < width; x += 4) {
				...
				long alphaBits = ((long)(blockData[blockIndex + 7] & 0xFF) << 56) |
						((long)(blockData[blockIndex + 6] & 0xFF) << 48) |
						((long)(blockData[blockIndex + 5] & 0xFF) << 40) |
						((long)(blockData[blockIndex + 4] & 0xFF) << 32) |
						((long)(blockData[blockIndex + 3] & 0xFF) << 24) |
						((long)(blockData[blockIndex + 2] & 0xFF) << 16) |
						((long)(blockData[blockIndex + 1] & 0xFF) << 8) |
						(blockData[blockIndex] & 0xFF);
				...
				long bits = ((long)(blockData[blockIndex + 15] & 0xFF) << 24) |
						((blockData[blockIndex + 14] & 0xFF) << 16) |
						((blockData[blockIndex + 13] & 0xFF) << 8) |
						(blockData[blockIndex + 12] & 0xFF);
	...
	private static void decompressDXT5(ImgData data, byte[] out) {
		...
		for (int y = 0; y < height; y += 4) {
			for (int x = 0; x < width; x += 4) {
				...
				long alphaBits = ((long)(blockData[blockIndex + 7] & 0xFF) << 40) |
						((long)(blockData[blockIndex + 6] & 0xFF) << 32) |
						((long)(blockData[blockIndex + 5] & 0xFF) << 24) |
						((long)(blockData[blockIndex + 4] & 0xFF) << 16) |
						((long)(blockData[blockIndex + 3] & 0xFF) << 8) |
						(blockData[blockIndex + 2] & 0xFF);
	...
	private static int getAlphaDXT5(int alpha0, int alpha1, int code) {
		if (alpha0 > alpha1) {
			switch (code) {
				...
				default: return (1 * alpha0 + 6 * alpha1) / 7;
			}
		} else {
			switch (code) {
				case 0: return alpha0;
	...
	public static class DDSImage {
		...
		public BufferedImage toBufferedImage() {
			...
			for (int i = 0; i < argb.length; i++) {
				int base = i * 4;
				argb[i] = ((pixels[base + 3] & 0xFF) << 24) |
						((pixels[base]     & 0xFF) << 16) |
						((pixels[base + 1] & 0xFF) << 8) |
						(pixels[base + 2]  & 0xFF);
			}
	...
	private static void writeDDSHeaderLe(OutputStream out, int width, int height, int fourCC) throws IOException {
		...
		// Reserved (11 ints)
		for (int i = 0; i < 11; i++) buffer.putInt(0);
	...
	private static void writeDDSHeaderWithDX10BC7(OutputStream out, int width, int height) throws IOException {
		...
		for (int i = 0; i < 11; i++) buffer.putInt(0); // Reserved

		buffer.putInt(32); // size
		buffer.putInt(0x00000004); // DDPF_FOURCC
		buffer.putInt(DX10); // FOURCC code for DX10
		buffer.putInt(0); buffer.putInt(0); buffer.putInt(0); buffer.putInt(0); buffer.putInt(0);
	...
	private static void compressDXT1Block(int[] colors, byte[] output, int offset) {
		...
		for (int c : colors) {
			...
			if (rgb < (minColor & 0x00FFFFFF)) minColor = c;
			if (rgb > (maxColor & 0x00FFFFFF)) maxColor = c;
		}
...
class BC7Util {
	public static void decompressBC7(ImgData data, byte[] out) {
		...
		for (int by = 0; by < (height + 3) / 4; by++) {
			for (int bx = 0; bx < (width + 3) / 4; bx++) {
				...
				if (blockOffset + 16 > blockData.length) break;
	...
	private static int[] decodeBC7Block(byte[] block) {
		...
		for (int s = 0; s < numSubsets; s++) { r[s][0] = reader.read(colorBits); r[s][1] = reader.read(colorBits); }
		for (int s = 0; s < numSubsets; s++) { g[s][0] = reader.read(colorBits); g[s][1] = reader.read(colorBits); }
		for (int s = 0; s < numSubsets; s++) { b[s][0] = reader.read(colorBits); b[s][1] = reader.read(colorBits); }
		if (alphaBits > 0) {
			for (int s = 0; s < numSubsets; s++) { a[s][0] = reader.read(alphaBits); a[s][1] = reader.read(alphaBits); }
		}
		...
		if (mode == 0 || mode == 3 || mode == 6 || mode == 7) {
			...
			for (int s = 0; s < numSubsets; s++) {
				int p = reader.read(1);
				pBits[s][0] = p; pBits[s][1] = p;
			}
		}

		// 4. Expand Endpoints to 8-bit
		for (int s = 0; s < numSubsets; s++) {
			r[s][0] = expand(r[s][0], colorBits, pBits[s][0]); r[s][1] = expand(r[s][1], colorBits, pBits[s][1]);
			g[s][0] = expand(g[s][0], colorBits, pBits[s][0]); g[s][1] = expand(g[s][1], colorBits, pBits[s][1]);
			b[s][0] = expand(b[s][0], colorBits, pBits[s][0]); b[s][1] = expand(b[s][1], colorBits, pBits[s][1]);
			if (alphaBits > 0) {
				a[s][0] = expand(a[s][0], alphaBits, pBits[s][0]); a[s][1] = expand(a[s][1], alphaBits, pBits[s][1]);
			} else {
				a[s][0] = 255; a[s][1] = 255;
			}
		}
		...
		for (int i = 0; i < 16; i++) {
			...
			if (rotation == 1) { int temp = finalA; finalA = finalR; finalR = temp; }
			if (rotation == 2) { int temp = finalA; finalA = finalG; finalG = temp; }
			if (rotation == 3) { int temp = finalA; finalA = finalB; finalB = temp; }
	...
	private static int expand(int val, int bits, int pBit) {
		...
		if (totalBits == 4) return (combinedBits << 4) | combinedBits;
		if (totalBits == 5) return (combinedBits << 3) | (combinedBits >> 2);
		if (totalBits == 6) return (combinedBits << 2) | (combinedBits >> 4);
		if (totalBits == 7) return (combinedBits << 1) | (combinedBits >> 6);
		return combinedBits;
	}
	...
	private static int interpolate(int e0, int e1, int index, int indexBits) {
		...
		if (indexBits == 3) w = weights3[index];
		if (indexBits == 4) w = weights4[index];
	...
	private static int[] getAnchors(int[] partition, int numSubsets) {
		...
		if (numSubsets > 1) {
			for (int i = 0; i < 16; i++) if (partition[i] == 1) { anchors[1] = i; break; }
		}
		if (numSubsets > 2) {
			for (int i = 0; i < 16; i++) if (partition[i] == 2) { anchors[2] = i; break; }
		}
		return anchors;
	}
	...
	private static int[] getBC7Partition(int numSubsets, int partitionIdx) {
		...
		if (numSubsets == 1) return partition;
	...
	private static void compressBC7BlockMode6(int[] blockColors, byte[] output, int offset) {
		...
		int e0_a8 = e0_a7 << 1; int e0_r8 = e0_r7 << 1; int e0_g8 = e0_g7 << 1; int e0_b8 = e0_b7 << 1;
		int e1_a8 = e1_a7 << 1; int e1_r8 = e1_r7 << 1; int e1_g8 = e1_g7 << 1; int e1_b8 = e1_b7 << 1;
		...
		if (p0_idx >= 8) {
			...
			int t = e0_a7; e0_a7 = e1_a7; e1_a7 = t;
			t = e0_r7; e0_r7 = e1_r7; e1_r7 = t;
			t = e0_g7; e0_g7 = e1_g7; e1_g7 = t;
			t = e0_b7; e0_b7 = e1_b7; e1_b7 = t;

			t = e0_a8; e0_a8 = e1_a8; e1_a8 = t;
			t = e0_r8; e0_r8 = e1_r8; e1_r8 = t;
			t = e0_g8; e0_g8 = e1_g8; e1_g8 = t;
			t = e0_b8; e0_b8 = e1_b8; e1_b8 = t;
		}
		...
		bw.write(e0_r7, 7); bw.write(e1_r7, 7);
		bw.write(e0_g7, 7); bw.write(e1_g7, 7);
		bw.write(e0_b7, 7); bw.write(e1_b7, 7);
		bw.write(e0_a7, 7); bw.write(e1_a7, 7);

		bw.write(0, 1); bw.write(0, 1); // P-Bits (set to 0)
	...
	private static int findClosestBC7Index(int color, int r0, int g0, int b0, int a0, int r1, int g1, int b1, int a1, int[] weights) {
		...
		for (int i = 0; i < 16; i++) {
			...
			int da = a - ia; int dr = r - ir; int dg = g - ig; int db = b - ib;
	...
	private static int colorDistance(int c1, int c2) {
		...
		int dr = r1 - r2; int dg = g1 - g2; int db = b1 - b2; int da = a1 - a2;
...
class BC7BlockData {
	...
	BC7BlockData() {
		endpoints = new int[3][2];
		alphaEndpoints = new int[3][2];
		endpointPBit = new int[3][2];
		indices = new int[16];
		colorBits = new int[3];
		alphaBits = new int[3];
	}
...
class ImgData {
	int h; int w; int code; int length; byte[] data; DDSHeaderDXT10 header;

	public ImgData(int h, int w, int code, int length, DDSHeaderDXT10 header) {
		this.h = h; this.w = w; this.code = code; this.length = length; this.header = header;
	}
}
