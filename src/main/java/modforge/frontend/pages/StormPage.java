	...
	static final class RuleWizardDialog extends JDialog {
		...
		RuleWizardDialog(Frame owner, StormRule existing, StormService stormService) {
			...
			if (existing != null) {
				...
				for (var s : existing.getSelectors())
					workingRule.getSelectors().add(s.deepCopy());
				for (var o : existing.getOperations())
					workingRule.getOperations().add(o.deepCopy());
			}
				...
				for (String type : new String[] { "selector", "and", "or", "not" }) {
					...
					JButton childAddBtn = miniBtn("+" + type, e -> {
						...
						if (! type.equals("selector"))
							child.getChildren().add(new GenericSelector());
