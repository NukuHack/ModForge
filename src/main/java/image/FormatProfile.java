 ...
 *
 *  _dif / _d      → DIFFUSE        (colour map, sRGB, BC1 no-alpha or BC3 with alpha)
 ...
 *  _emissive / _e → EMISSIVE       (additive colour, BC1)
 *  _mask / _msk   → MASK           (binary / multi-channel mask, BC3)
...
public enum FormatProfile {
	...
	NORMAL("Normal Map", TargetFormat.BC5, false, "Tangent-space XY normal map. KCD2 expects BC5_UNORM (two-channel RG)."),
	SPECULAR("Specular / Roughness", TargetFormat.BC5, false, "Specular / roughness data packed into RG. BC5_UNORM gives best precision."),
	GLOSS("Gloss Map", TargetFormat.BC3, true, "Gloss channel – often stored in the alpha of a normal map. BC3 or separate BC4."),
	AMBIENT_OCC("Ambient Occlusion", TargetFormat.BC1, false, "Grayscale AO map. BC1 is sufficient; no alpha needed."),
	EMISSIVE("Emissive", TargetFormat.BC1, false, "Additive emissive colour. BC1 is standard."),
	MASK("Mask", TargetFormat.BC3, true, "Multi-channel mask (e.g. damage, wetness). BC3 preserves the alpha channel."),
	ID_MAP("ID Map", TargetFormat.UNCOMPRESSED_RGBA, true, "Colour-ID map. Must stay uncompressed (RGBA8) so IDs are exact."),
	UNKNOWN("Unknown / General", TargetFormat.BC7, true, "Format unknown. Defaulting to BC7 (best quality, supports alpha).");
	...
	public enum TargetFormat {
		BC1("BC1 / DXT1 – 4 bpp, no alpha"),
		BC3("BC3 / DXT5 – 8 bpp, full alpha"),
		BC5("BC5 – 8 bpp, two-channel RG (normal/spec)"),
		BC7("BC7 – 8 bpp, high-quality RGBA"),
		UNCOMPRESSED_RGBA("Uncompressed RGBA8 – 32 bpp, lossless");
