...
import com.nukuhack.modforge.backend.service.BaseServiceTest;
...
class DDSUtilTest extends BaseServiceTest {
	...
	void testRoundTripDXT1() throws IOException {
		...
		assertTrue(differences <= maxDifferences, "Too many pixels differ significantly: " + differences + " of " + samplesToCheck);
	}
	...
	void testWriteAllDDSFormats() throws IOException {
		...
		System.out.printf("DXT1 DDS: %d bytes (%.1f%% of PNG)%n", Files.size(dxt1Path), (Files.size(dxt1Path) * 100.0 / Files.size(pngPath)));
		System.out.printf("DXT3 DDS: %d bytes (%.1f%% of PNG)%n", Files.size(dxt3Path), (Files.size(dxt3Path) * 100.0 / Files.size(pngPath)));
		System.out.printf("DXT5 DDS: %d bytes (%.1f%% of PNG)%n", Files.size(dxt5Path), (Files.size(dxt5Path) * 100.0 / Files.size(pngPath)));
	...
	private void compareAlphaQuality(DDSUtil.DDSImage original, DDSUtil.DDSImage converted, String formatName) {
		...
		System.out.printf("%s: Avg Alpha Error = %.2f, Max Alpha Error = %d%n", formatName, avgError, maxAlphaError);
	}
	...
	void testMemoryFootprint() throws IOException {
		...
		assertTrue(memoryGrowth < 5 * 1024 * 1024, "Memory growth too high: " + memoryGrowth + " bytes");
	}
}